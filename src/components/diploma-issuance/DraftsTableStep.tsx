...
export default function DraftsTableStep({
  ...
  const getClassificationBadge = (classification: string) => {
    const badges: Record<string, { label: string; color: string }> = {
      EXCELLENT: { label: '🏆 Xuất sắc', color: 'bg-purple-500/20 text-purple-300 border-purple-500/50' },
      GOOD: { label: '🥇 Giỏi', color: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/50' },
      CREDIT: { label: '🥈 Khá', color: 'bg-blue-500/20 text-blue-300 border-blue-500/50' },
      AVERAGE: { label: '🥉 TB', color: 'bg-green-500/20 text-green-300 border-green-500/50' },
      FAIL: { label: '❌ Yếu', color: 'bg-red-500/20 text-red-300 border-red-500/50' },
    }
        ...
        <table className="w-full text-sm">
          <thead className="bg-white/5 border-b border-white/10">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-semibold">Xếp loại</th>
            ...
            {drafts.length === 0 ? (
              ...
              drafts.map((draft, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-mono text-blue-400">{draft.studentId}</td>
                  <td className="px-4 py-3 font-semibold">{draft.studentName}</td>
                  <td className="px-4 py-3 text-gray-300 text-xs">{draft.studentEmail}</td>
                  <td className="px-4 py-3 text-gray-300">{draft.studentPhone}</td>
                  <td className="px-4 py-3 text-gray-300">{draft.studentClass}</td>
                  <td className="px-4 py-3 text-gray-300">{draft.faculty}</td>
                  <td className="px-4 py-3 font-bold text-yellow-400">{draft.GPA.toFixed(2)}</td>
                  <td className="px-4 py-3">{getClassificationBadge(draft.classification)}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button
                        ...
                        disabled={draft.isMinted || loading || deletingId === draft.id}
